        ...
        if(in_array($fileActualExt, $allowed)){
            if($fileError === 0) {
                ...
                if($fileSize < 5000000)/*5000000 = 5mb */{
                    ...
                    header("Location: adultnomination.php?error=file_size>5MB");
                    //echo 'Your file is too big! Try uploading another file!'; // doesn't even appear rip b/c this happens after upload ....
                }
            } else if($fileError === 1) {
                //1 means error uploading
                header("Location: nomination.php?error=can't_upload");
                //echo 'There was an error uploading your file';
            }
        } else {
            header("Location: nomination.php?error=wrong_file_type");
